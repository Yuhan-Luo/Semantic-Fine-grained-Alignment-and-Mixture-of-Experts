...
# from k_organ_MOE1 import CLIPVisionTransformer_K
from detectors.k_organ_MOE1 import CLIPVisionTransformer_K

logger = logging.getLogger(__name__)


@DETECTOR.register_module(module_name='clip_fine_text_MSE_moe_k')
class CLIP16x16Segmentor_MSE_moe_k(AbstractDetector):
    def __init__(self, config):
        ...
        #self.model.vision_model = CLIPVisionTransformer_K(vision_cfg).to(self.device)  # 替换本来的vision_model
        #self.model.vision_model.config = vision_cfg
    ...
    # def build_moe_model(self, model):
    #     layers = range(5,12)
    #     for layer_indice in layers:
    #         mlp = model.vision_model.encoder.layers[layer_indice].mlp
    #         new_mlp = Moe()
    #         for expert in new_mlp.experts:
    #             expert[0].weight.data.copy_(mlp.fc1.weight.data)
    #             expert[0].bias.data.copy_(mlp.fc1.bias.data)
    #             expert[2].weight.data.copy_(mlp.fc1.weight.data)
    #             expert[2].bias.data.copy_(mlp.fc1.bias.data)
    #         model.vision_model.encoder.layers[layer_indice].mlp = new_mlp
    #     return model
    ...
    def get_image_patch_features(self, image, landmarks):
        if isinstance(image, torch.Tensor) and image.device != self.device:
            image = image.to(self.device)

        # 这里假设 data_dict["image"] 已经按 CLIP 要求预处理了
        organ_mask = self.get_organ_mask(landmarks)
        #print(organ_mask[0])
        #print(self.model.vision_model)
        #input()
        

        outputs = self.model.vision_model.forward(image, output_hidden_states=True, organ_mask=organ_mask)
    ...
    def get_organ_mask(self, landmarks):  # landmarks [2B,81,2]
        B = landmarks.shape[0]
        img_size = 224
        patch_size = 16
        num_patches = (img_size // patch_size) ** 2  # 14×14=196
        organ_mask = torch.zeros((B, num_patches), dtype=torch.int32, device=landmarks.device)

        pt_patch = (landmarks / patch_size).floor().long()  # [2B,81,2],0-13 记录横纵各在第几块上
        pt_patch_indice = pt_patch[..., 1] * (img_size // patch_size) + pt_patch[..., 0]  # [2B, 81] 记录这个点是第几个块

        organ_keypts = {
            1: list(range(17, 28)),  # 眉毛17-27
            2: list(range(28, 36)),  # 鼻子28-35
            3: list(range(36, 48)),  # 眼睛36-47
            4: list(range(48, 68))  # 嘴巴48-67
        }

        for organ_id, pts in organ_keypts.items():
            for b in range(B):
                organ_patches = pt_patch_indice[b, pts].unique() # 去重多个点占一个块时只取一次
                organ_mask[b, organ_patches] = organ_id  # 如果不同的器官在同一个块上，只取一种

        cls_mask = torch.zeros((B, 1), dtype=torch.int32, device=landmarks.device)
        organ_mask = torch.cat((cls_mask, organ_mask), dim=1)  # [2B,197]
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        ...
        landmarks = data_dict['landmark']  # np格式[2B,81,2]
        patch_features, cls_features = self.features(images, landmarks)
    ...
    def features(self, x, landmarks):
        return self.get_image_patch_features(x, landmarks)
    ...
    def get_losses(self, data_dict: dict, pred_dict: dict) -> dict:
        ...
        if self.training:
            ...
            loss = L_rank_intra * 0.3 + L_rank_real_fake * 0.2 + loss
