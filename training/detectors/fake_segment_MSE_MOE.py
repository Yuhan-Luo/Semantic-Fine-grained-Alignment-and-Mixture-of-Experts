...
class Moe(nn.Module):
    def __init__(self, hidden_size=768, n_experts=8, topk=2):
        super().__init__()
        self.hidden_size = hidden_size
        self.n_experts = n_experts
        self.topk = topk
        self.gate = nn.Linear(hidden_size, n_experts)
        self.experts = nn.ModuleList([nn.Sequential(nn.Linear(hidden_size, 4*hidden_size),
                                                    nn.GELU(),nn.Linear(4*hidden_size, hidden_size))for _ in range(n_experts)])
    def forward(self,x):  # x [B,197,768]
        gate_scores = self.gate(x)  # [B,197,4]
        topk_weights, topk_indices = torch.topk(gate_scores, self.topk, dim=-1)  #indices [B,197,topk]记录每个patch选择的专家
        topk_weights = nn.functional.softmax(topk_weights, dim=-1)
        feat = torch.zeros_like(x)
        for i in range(self.topk):  # 按顺序处理每个patch的第一选择专家
            expert_indices = topk_indices[...,i]  # [B,197]取每个patch当前专家号
            weights = topk_weights[...,i].unsqueeze(-1)  # [B,197,1]
            for expert_indice in range(self.n_experts):
                expert_mask = (expert_indices == expert_indice)  #[B,197]
                if expert_mask.any():
                    temp_feat = self.experts[expert_indice](x[expert_mask])
                    feat[expert_mask] += weights[expert_mask] * temp_feat
        return feat

@DETECTOR.register_module(module_name='clip_fine_text_MSE_moe')
class CLIP16x16Segmentor_MSE_moe(AbstractDetector):
