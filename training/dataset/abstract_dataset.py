...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def get_landmarks(img):
        ...
        faces = DeepfakeAbstractBaseDataset.detector(gray_img)
        if len(faces) == 0:
            return np.zeros((81, 2), dtype=np.int32)  # 没检测到脸就返回全0landmarks

        shape = DeepfakeAbstractBaseDataset.predictor(gray_img, faces[0])  # 有多个脸只处理第一个
    ...
    def collate_fn(batch):
        ...
        landmarks = []
        for img in images:
            landmark = DeepfakeAbstractBaseDataset.get_landmarks(img)
