...
class Trainer(object):
    ...
    def train_step(self,data_dict):
        if self.config['optimizer']['type']=='sam':
            ...
            if(True):
                ...
                loss_main = losses['overall']
                loss_patch = losses['loss_patch']
                self._apply_apc_grad_two_losses(loss_main,loss_patch)
                self.optimizer.step()
    ...
    def train_epoch(
        ...
        for iteration, data_dict in tqdm(enumerate(train_data_loader),total=len(train_data_loader)):
            ...
            for name, value in losses.items():
                train_recorder_loss[name].update(value.detach().item())
    ...
    def _apply_apc_grad_two_losses(self, loss_main, loss_patch):
        """
        APCGrad 协调 overall / loss_patch：
        - 不做 forward
        - 不在这里 step
        - 不用 retain_graph 导致显存疯涨
        """

        # 统一拿参数（兼容 DDP）
        if isinstance(self.model, DDP):
            params = [p for p in self.model.module.parameters() if p.requires_grad]
        else:
        ...
        grads_main = torch.autograd.grad(
            loss_main,
            params,
            retain_graph=True,    # 保留计算图，后面还要对 loss_patch 求梯度
            create_graph=False,
            allow_unused=True,
        )

        # 2) 第二套梯度：loss_patch
        grads_patch = torch.autograd.grad(
            loss_patch,
            params,
            retain_graph=False,   # 用完就把图扔了，显存不积累
            create_graph=False,
            allow_unused=True,
        )
        ...
        norm1_sq = 0.0
        norm2_sq = 0.0
        for g1, g2 in zip(grads_main, grads_patch):
        ...
        dot = dot.item()
        norm1_sq = norm1_sq.item()
        norm2_sq = norm2_sq.item()
        eps = 1e-12

        for p, g1, g2 in zip(params, grads_main, grads_patch):
            ...
            if g1 is None:
                ...
                if dot < 0 and norm1_sq > eps and norm2_sq > eps:
                   # 有冲突，做双向投影
                    proj1 = dot / (norm2_sq + eps)
                    proj2 = dot / (norm1_sq + eps)
                    g1_proj = g1 - proj1 * g2
                    g2_proj = g2 - proj2 * g1
                    g = 0.5 * (g1_proj + g2_proj)
                else:
                    # 没冲突，简单平均
                    g = 0.5 * (g1 + g2)

            if g is not None:
                p.grad = g.detach()   # 写回 p.grad，注意 detach，别挂图
